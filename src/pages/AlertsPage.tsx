  ...
  const renderWhaleEvent = (ev: EventRow) => {
    ...
    return (
      <div key={ev.id} className="flex flex-wrap sm:flex-nowrap items-start sm:items-center gap-2 sm:gap-4 px-3 sm:px-4 py-3 border border-white/[0.04] rounded-lg hover:bg-white/[0.02] transition-colors">
        ...
        <div className="font-mono text-xs tracking-wider font-bold" style={{ color: "rgba(255,215,0,0.8)" }}>
          🐋 {label}
        </div>
        <div className="font-mono text-xs font-bold" style={{ color: dirColor }}>
          {e?.direction === "OUT" ? "SORTIE" : "ENTRÉE"} {amount}
        </div>
        {e?.counterparty && (
          <div className="font-mono text-[10px] text-white/25 truncate max-w-[120px]">
            → {e.counterparty.slice(0, 8)}…
          </div>
        )}
        <div className="font-mono text-[10px] text-white/20 flex-shrink-0 ml-auto">
