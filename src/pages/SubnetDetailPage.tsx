    ...
    <div className="h-full w-full bg-background text-foreground overflow-auto pb-24">
      <div className="px-4 sm:px-6 py-6 max-w-[1100px] mx-auto space-y-7">
        ...
        {decision.taoFluteStatus?.taoflute_match && (
          <SectionCard>
            <SectionTitle icon="💀" title={fr ? "Risque externe TaoFlute" : "TaoFlute External Risk"} />
            <div className="px-5 py-4 space-y-3">
              <div className="flex items-center gap-3">
                {decision.taoFluteStatus.taoflute_severity === "priority" ? (
                  ...
                  <span className="font-mono text-[11px] font-bold px-2.5 py-1 rounded" style={{ background: "hsla(var(--signal-go-spec), 0.1)", color: "hsl(var(--signal-go-spec))", border: "1px solid hsla(var(--signal-go-spec), 0.2)" }}>
                    WATCH — {fr ? "SOUS SURVEILLANCE" : "UNDER WATCH"}
                  </span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg px-3 py-2 bg-muted/20 border border-border">
                  ...
                  <div className="font-mono text-[11px] text-foreground/80 mt-0.5">
                    {decision.taoFluteStatus.externalRisk?.source_ref === "confirmed_fallback" ? (fr ? "Fallback confirmé" : "Confirmed fallback") : "TaoFlute"}
                  </div>
                </div>
                <div className="rounded-lg px-3 py-2 bg-muted/20 border border-border">
                  <div className="font-mono text-[7px] text-muted-foreground tracking-widest uppercase">{fr ? "SÉVÉRITÉ" : "SEVERITY"}</div>
                  <div className="font-mono text-[11px] text-foreground/80 mt-0.5 uppercase">
                    {decision.taoFluteStatus.taoflute_severity}
                  </div>
                </div>
              </div>
              {decision.taoFluteStatus.externalRisk && (decision.taoFluteStatus.externalRisk.liq_price != null || decision.taoFluteStatus.externalRisk.liq_haircut != null) && (
                <div className="grid grid-cols-2 gap-3">
                  {decision.taoFluteStatus.externalRisk.liq_price != null && (
                    <div className="rounded-lg px-3 py-2 bg-muted/20 border border-border">
                      <div className="font-mono text-[7px] text-muted-foreground tracking-widest uppercase">LIQ PRICE</div>
                      <div className="font-mono text-[11px] text-foreground/80 mt-0.5">{decision.taoFluteStatus.externalRisk.liq_price.toFixed(4)}</div>
                    </div>
                  )}
                  {decision.taoFluteStatus.externalRisk.liq_haircut != null && (
                    <div className="rounded-lg px-3 py-2 bg-muted/20 border border-border">
                      <div className="font-mono text-[7px] text-muted-foreground tracking-widest uppercase">LIQ HAIRCUT</div>
                      <div className="font-mono text-[11px] text-foreground/80 mt-0.5">{(decision.taoFluteStatus.externalRisk.liq_haircut * 100).toFixed(1)}%</div>
                    </div>
                  )}
                </div>
              )}
              <div className="rounded-lg px-4 py-2.5 border border-destructive/15 bg-destructive/[0.03]">
                ...
                <div className="font-mono text-[10px] text-foreground/70 leading-relaxed">
                  {decision.taoFluteStatus.taoflute_severity === "priority"
                    ? (fr ? "Ce subnet est en priorité TaoFlute. Entrée bloquée. Seules les actions SORTIR et ÉVITER sont autorisées."
                        : "This subnet is TaoFlute priority. Entry blocked. Only EXIT and AVOID actions allowed.")
                    : (fr ? "Ce subnet est sous surveillance TaoFlute. Entrée agressive interdite."
                        : "This subnet is under TaoFlute watch. Aggressive entry blocked.")}
                </div>
              </div>
            </div>
          </SectionCard>
