...
describe("evaluateRawState", () => {
  ...
  it("returns DEPEG_CONFIRMED when delistScore >= depegEnter threshold", () => {
    const d = makeDecision({ delistScore: 50 });
    expect(evaluateRawState(d, "ALIGNED", s)).toBe("DEPEG_CONFIRMED");
  });

  it("returns DEPEG_HIGH_RISK for HIGH_RISK_NEAR_DELIST with score", () => {
    const d = makeDecision({ delistCategory: "HIGH_RISK_NEAR_DELIST", delistScore: 30 });
...
describe("DecisionStateManager — Delta trigger", () => {
  it("allows re-firing during cooldown if score delta exceeds threshold", () => {
    ...
    const d1 = makeDecision({ delistCategory: "DEPEG_PRIORITY", delistScore: 50 });
    mgr.tick(d1, "ALIGNED", 1000);

    // Return to OK
    const dOk = makeDecision({ delistScore: 5 });
    mgr.tick(dOk, "ALIGNED", 2000);

    // Re-fire with delistScore=70 (delta = |70-50|/100 = 0.20 > 0.15)
    const d2 = makeDecision({ delistCategory: "DEPEG_PRIORITY", delistScore: 70 });
  ...
  it("suppresses when delta is below threshold", () => {
    ...
    const d2 = makeDecision({ delistCategory: "DEPEG_PRIORITY", delistScore: 55 });
...
describe("DecisionStateManager — tickAll", () => {
  it("processes multiple subnets in one call", () => {
    ...
    const decisions = [
      ...
      makeDecision({ netuid: 3, delistCategory: "DEPEG_PRIORITY", delistScore: 90 }),
    ];
