...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const s of subnets) {
      ...
      // TMC returns liquidity in rao (1 TAO = 1e9 rao) — convert to TAO
      const rawLiq = Number(s.tao_liquidity ?? s.liquidity ?? 0) || 0;
      const liquidity = rawLiq > 1e6 ? rawLiq / 1e9 : rawLiq || null;
